...
class CollectionUtility
{
     ...
     */
    public static function keepKeys(array $collection, $keys, $removal_action = self::REMOVAL_ACTION_DELETE): array
    {
     ...
     * @throws \InvalidArgumentException
     */
    public static function removeKeys(array $collection, $keys, $removal_action = self::REMOVAL_ACTION_DELETE): array
    {
     ...
     *                                         "overwritten"=>[Items that were successfully keyed but subsequently
     *                                         overwritten],
     *                                         "invalid"=>[Items that did not have required properties specified]
     *                                         ]
     * @return array A collection keyed by the provided property(s).
     */
    public static function keyByProperty(array $collection, $property_to_key_by, $return_meta = false): array
    {
        $result = [];
        $properties_to_key_by = (array)$property_to_key_by;
     ...
     */
    public static function groupByProperty(array $collection, $property_to_key_by): array
    {
        $result = [];
        $properties_to_key_by = (array)$property_to_key_by;
        foreach ($collection as $item) {
     ...
     * @param array  $collection    A collection of arrays or objects that can be cast to arrays (see
     *                              castObjectToArray). Any values that are not arrays will be automatically filtered
     *                              out.
     ...
     * @throws \InvalidArgumentException
     */
    public static function findFirstWhere(
        array $collection,
        array $property_list,
     ...
     * @param array  $collection             A collection of arrays. Any values that are not arrays will be
     *                                       automatically filtered out.
     * @param array  $property_list          An array of properties that must match for the item to be included in the
     *                                       final list. Comparison is run using ArrayUtility::dotRead Can also be
     *                                       provided as an array of arrays. If passed in this format, each array is
     *                                       treated as part of an "OR" test i.e. if any individual array would pass
     *                                       the filter test if submitted on its own, the item is considered valid.
     * @param string $match_type             Any of the available MATCH_TYPE_* constants or null to use default.
     *                                       Affects whether value comparisons are performed on exact data types.
     *                                       Default is "loose" which does not perform a type check
    ...
    public static function filterWhere(
        ...
        array $property_list,
        $match_type = self::MATCH_TYPE_LOOSE,
        $force_key_preservation = false
    ): array {
        if ($match_type === null) {
            $match_type = self::MATCH_TYPE_LOOSE;
        }
        if (!\in_array($match_type,
            [self::MATCH_TYPE_LOOSE, self::MATCH_TYPE_STRICT, self::MATCH_TYPE_CASE_INSENSITIVE], true)) {
     ...
     * @param array  $collection    A collection of arrays. Any values that are not arrays will be automatically
     *                              filtered out.
     ...
     * @throws \InvalidArgumentException
     */
    public static function findFirstWhereNot(
        array $collection,
        array $property_list,
        $match_type = self::MATCH_TYPE_LOOSE,
        $return_key = false
     ...
     * @param array  $collection             A collection of arrays. Any values that are not arrays will be
     *                                       automatically filtered out.
     * @param array  $property_list          An array of properties that must NOT match for the item to be included in
     *                                       the final list. Comparison is run using ArrayUtility::dotRead
     * @param string $match_type             Any of the available MATCH_TYPE_* constants. Affects whether value
     *                                       comparisons are performed on exact data types.
     * @param bool   $force_key_preservation Keys are preserved for assoc arrays only by default. Set to true to ALWAYS
     *                                       preserve keys
     * @return array
     * @throws \InvalidArgumentException
     */
    public static function filterWhereNot(
        array $collection,
        array $property_list,
        $match_type = self::MATCH_TYPE_LOOSE,
        $force_key_preservation = false
    ...
    public static function sort(
        array $array,
        $property,
        $sort_direction = self::SORT_DIRECTION_ASCENDING,
     ...
     * @throws \InvalidArgumentException
     */
    public static function asort(
        array $array,
        $property,
        $sort_direction = self::SORT_DIRECTION_ASCENDING,
        $sort_flags = SORT_REGULAR
    ): array {
        $sort_target = static::pluck($array, $property);
        if ($sort_direction === self::SORT_DIRECTION_ASCENDING || $sort_direction === null) {
     ...
     * @return array
     */
    public static function pluck(array $collection, $property): array
    {
