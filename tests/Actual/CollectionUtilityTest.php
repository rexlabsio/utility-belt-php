...
class CollectionUtilityTest extends TestCase
{
    ...
    public function testThatKeysCanBeRemovedInArray()
    {
        ...
        $kept_keys = CollectionUtility::removeKeys($special_items, ["animal", "weather"],
            CollectionUtility::REMOVAL_ACTION_DELETE);
        $this->assertEquals([
            ["name" => "John"],
            ["name" => "William"],
        ], $kept_keys);
    ...
    public function testThatCollectionCanBeFilteredByProperties()
    {
        ...
        $filtered = CollectionUtility::filterWhere($items, ["a" => "b"],
            CollectionUtility::MATCH_TYPE_CASE_INSENSITIVE);
        $this->assertCount(2, $filtered);
